...
fn compute_energized_cells(grid: &Vec<Vec<Cell>>, x: usize, y: usize, dir: Direction) -> usize {
    let mut expanded_beams = HashSet::new();
    let mut beams_to_process = VecDeque::new();
    beams_to_process.push_back((x, y, dir));
...
fn main() {
    ...
    inputs.extend((0..grid[0].len()).map(|x| (x, 0, Direction::South)));
    inputs.extend((0..grid[0].len()).map(|x| (x, grid.len() - 1, Direction::North)));
    inputs.extend((0..grid.len()).map(|y| (0, y, Direction::East)));
    inputs.extend((0..grid.len()).map(|y| (grid[0].len() - 1, y, Direction::West)));

    let max = inputs
        .into_iter()
        .map(|(x, y, dir)| compute_energized_cells(&grid, x, y, dir))
        .max()
        .unwrap();
