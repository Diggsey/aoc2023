...
fn main() {
    ...
    for i in 0..map.len() - 1 {
        for j in (i + 1)..map.len() {
            ...
            let non_expanding_cols = populated_columns.range(xmin..xmax).count() as i32;
            let non_expanding_rows = populated_rows.range(ymin..ymax).count() as i32;
            dist += base_dist * 2 - non_expanding_cols - non_expanding_rows;
        }
    }
    println!("{dist}");
}
